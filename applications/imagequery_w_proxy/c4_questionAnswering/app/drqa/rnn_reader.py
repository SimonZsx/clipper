...
#   - X use gradient hook (instead of tensor copying) for gradient masking
#     YY: Gradient Hook usage: 
...
class RnnDocReader(nn.Module):
    ...
    def __init__(self, opt, padding_idx=0, embedding=None):
        ...
        # Input size to RNN: word embedding + question embedding + manual features
    ...
    def forward(self, x1, x1_f, x1_pos, x1_ner, x1_mask, x2, x2_mask):
        """Inputs:
        YY: len_d may means length of the document. x1: document, x2: question.
        ...
        if self.opt['dropout_emb'] > 0:
            ...
            # YY: self.training() is just the same mode set when we call model.train() or model.eval()
        ...
        doc_hiddens = self.doc_rnn(torch.cat(drnn_input_list, 2), x1_mask)
