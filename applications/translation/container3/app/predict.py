...
def batch_predict(paragraph_list):
    ids = np.zeros((len(paragraph_list), maxSeqLength), dtype='int32')
    ...
    for paragraph in paragraph_list:
        ...
        for word in paragraph:
            try:
                ids[list_index][indexCounter] = wordsList.index(word)
            except ValueError:
                    ids[list_index][indexCounter] = 399999 #Vector for unkown words
            indexCounter = indexCounter + 1
            if indexCounter >= maxSeqLength:
                break
    ...
    with tf.Session() as sess:
        ...
        for i in range(batchSize):
            lb.append([1,0])
            inputdt[i]=ids[i]
        Prediction=sess.run(correctPred, {input_data: inputdt, labels: lb})
        print("\n[INFO] Prediction: ", Prediction)
        sess.close()
        # if Prediction:
        #     return 1;
