...
class ProxyServerServicer(prediction_pb2_grpc.ProxyServerServicer):
    ...
    def downstream(self, request, context): ## Consumer 
       ...
       #logging.info("Start processing request...")
        ...
        ## Generate request
        temp_request = prediction_pb2.request(input_ = request.input_, src_uri="front-end", seq=temp_seq, req_id = self.request_id, timestamp = call_time )
...
def serve():
    ...
    proxy_name = os.environ["ENTRY_PROXY_NAME"]
    proxy_port = os.environ["ENTRY_PROXY_PORT"]
    max_workers_ = os.environ["MAX_WORKERS"]
    #redis_name = os.environ["REDIS_IP"]
    #redis_port = os.environ["REDIS_PORT"]

    server = grpc.server(futures.ThreadPoolExecutor(max_workers=int(max_workers_)))
